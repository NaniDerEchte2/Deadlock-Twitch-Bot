...
def main() -> None:
    parser = argparse.ArgumentParser(description="Create a test Stripe Checkout Session")
    parser.add_argument("--plan", default="analysis_dashboard", help="Plan ID (or alias)")
    parser.add_argument("--cycle", type=int, default=1, choices=[1, 6, 12], help="Billing cycle months")
    parser.add_argument("--email", default="test@example.com", help="Prefill customer email")
    args = parser.parse_args()

    plan_id = PLAN_ALIASES.get(args.plan, args.plan)
    cycle = args.cycle

    secret_key = _load_vault("STRIPE_SECRET_KEY") or _load_vault("TWITCH_BILLING_STRIPE_SECRET_KEY")
    if not secret_key:
        raise SystemExit("STRIPE_SECRET_KEY not in vault")

    price_map_raw = _load_vault("STRIPE_PRICE_ID_MAP") or _load_vault("TWITCH_BILLING_STRIPE_PRICE_ID_MAP")
    ...
    if not cycle_map:
        raise SystemExit(f"Plan '{plan_id}' not found in price map. Available: {list(price_map)}")
    ...
    print(f"  {session.url}")
    print()
    print("  After payment: webhook fires -> twitch_billing_events + twitch_billing_subscriptions updated")
